...
bool InitAll(ProgramContext *ctx)
{
...
void DeInitAll(ProgramContext *ctx)
{
...
{
    (void) argc; (void) argv;
    ProgramContext ctx = {0};
    if(!InitAll(&ctx)) {
        SDL_LogError(SDL_LOG_CATEGORY_ERROR, "Could not init program");
        return 1;
    }
    ...
    while(!quit) {
        ...
        Uint64 targetTicks = (Uint64)(1000000000.0f/targetFPS);
